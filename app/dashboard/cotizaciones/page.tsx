...
import { Plus, Trash2, FileText, User, ShoppingBag, Percent, Search } from "lucide-react";
...
export default function NuevaCotizacionPage() {
  ...
  const [descuentoInput, setDescuentoInput] = useState<number>(0);
  
  // 🔥 ESTADO PARA EL FILTRO DEL CARRITO (BÚSQUEDA INTERMEDIA)
  const [busquedaCarrito, setBusquedaCarrito] = useState<string>("");
  ...
  const agregarAlCarrito = () => {
    ...
    if (existe) {
      ...
      setCarrito([...carrito, {
        ...
        descuentoPorcentaje: descuentoInput
      }]);
    }
    setCantidadInput(1);
    setDescuentoInput(0);
  ...
  // 🔥 FILTRADO DEL CARRITO EN TIEMPO REAL
  const carritoFiltrado = carrito.filter((item) => {
    const termino = busquedaCarrito.toLowerCase();
    return (
      item.nombre.toLowerCase().includes(termino) ||
      item.codigoSku.toLowerCase().includes(termino)
    );
  });
  ...
  return (
    <div className="space-y-6 text-slate-900">
      ...
      <form onSubmit={manejarEnviarCotizacion} className="space-y-6">
        ...
        {/* 🔥 FILTRO DE BÚSQUEDA ADENTRO DEL CARRITO */}
        {carrito.length > 0 && (
          <div className="flex items-center max-w-xs relative mt-4">
            <Search className="absolute left-3 h-4 w-4 text-slate-400" />
            <input
              type="text"
              placeholder="Filtrar carrito por SKU o nombre..."
              className="w-full pl-9 pr-4 py-2 text-xs bg-white border border-slate-200 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 text-slate-900"
              ...
              {carritoFiltrado.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-6 py-12 text-center text-slate-400 italic">
                    {carrito.length === 0 ? "La lista de cotización está vacía." : "No hay coincidencias en la búsqueda."}
                  </td>
                </tr>
              ) : (
      ...
      {cotizacionEmitida && (
        <div className="mt-4 p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between animate-fade-in">
          <div>
            ...
            <p className="text-xs text-emerald-700">Cotización N° #000{cotizacionEmitida.id} emitida a las {cotizacionEmitida.horaEmision}.</p>
          </div>
